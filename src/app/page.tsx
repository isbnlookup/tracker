...
export default function Home() {
  ...
  return (
    <main id="app-main" className="min-h-[100dvh] bg-white text-slate-800 flex flex-col items-center py-8 sm:py-16 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-3xl w-full flex flex-col gap-8 sm:gap-10">

        {/* Header */}
        <header id="vault-header" className="text-center space-y-4">
          <div className="inline-flex items-center justify-center p-4 bg-[#0883c3]/10 rounded-full mb-2 sm:mb-4 ring-1 ring-[#0883c3]/30">
            <KeyIcon className="w-8 h-8 sm:w-10 sm:h-10 text-[#0883c3]" aria-hidden="true" />
          </div>
          <h1 id="vault-title" className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900">
            ISBN Code Vault
          </h1>
          <p id="vault-description" className="text-base sm:text-lg text-slate-600 max-w-xl mx-auto px-2">
        ...
        <section aria-label="Advertisement">
          <AdBanner dataAdSlot="7172316222" />
        </section>

        {/* Main Card */}
        <section id="vault-card-section" aria-labelledby="vault-title" className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-10 md:p-12 shadow-xl shadow-slate-200/50">

          <article className="flex flex-col items-center text-center space-y-8 sm:space-y-10">
            {error && (
            ...
            <div className="space-y-4 w-full">
              <label htmlFor="current-isbn" className="text-xs sm:text-sm font-bold text-[#0883c3] uppercase tracking-widest block text-left sm:text-center w-full">
                Your Unique ISBN Code
              </label>
              <div id="current-isbn" aria-live="polite" className="flex items-center justify-center bg-slate-50 border border-slate-200 rounded-2xl p-6 sm:p-10 min-h-[140px] shadow-inner transition-all">
                {loading ? (
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 w-full sm:w-auto">
                    <span id="revealed-isbn-text" className="text-3xl sm:text-5xl md:text-6xl font-mono font-bold tracking-wider text-slate-900 break-all text-center">
                      {currentIsbn}
                    </span>
                    <button
                      id="copy-isbn-btn"
        ...
        {/* History Section */}
        {history.length > 0 && (
          <section id="history-section" className="mt-4 sm:mt-8 space-y-4">
            <h2 id="history-title" className="text-lg sm:text-xl font-bold text-slate-800 flex items-center gap-3">
            ...
            <div id="history-list" aria-labelledby="history-title" className="bg-white border border-slate-200 rounded-2xl overflow-hidden divide-y divide-slate-100 shadow-sm">
              {history.map((item, index) => (
                <article key={item.isbn} id={`history-item-${item.isbn}`} className={`p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 transition-colors hover:bg-slate-50 ${currentIsbn === item.isbn ? 'bg-[#0883c3] bg-opacity-5 border-l-4 border-[#0883c3] shadow-inner' : 'border-l-4 border-transparent'}`}>
